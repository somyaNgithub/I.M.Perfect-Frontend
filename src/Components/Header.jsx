...
const Header = () => {
    ...
    <div className='flex sticky top-0 w-full mx-auto px-5 bg-gray-300 max-w-[96vw]  z-50 items-center gap-10  max-md:overflow-x-auto justify-between  '>
          <Link to={'/'} >
...
<div className='w-3/5 mx-auto flex justify-between gap-10'>
      ...
      <Link to ='/users'className='text-lg font-poppins font-medium text-TextColor_Neutral' >Users</Link>
          ...
          {user ?<div className='flex flex-col gap-3 justify-center items-center'>
            ...
            : <button onClick={() => navigate('/login')} className='px-3 whitespace-nowrap  items-center h-10 rounded-lg bg-Secondary1_Neutral text-white'>Login/sign up</button>}
